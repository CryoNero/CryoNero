...
crypto::Hash cryonero::proof_of_trust::get_hash() const {
	std::string s;
...
namespace seria {
	...
	void ser(cryonero::UUID &v, seria::ISeria &s) { s.binary(&v, sizeof(v)); }

	void ser_members(cryonero::PeerlistEntry &v, seria::ISeria &s) {
	...
	void ser_members(cryonero::NetworkAddress &v, seria::ISeria &s) {
		seria_kv("ip", v.ip, s);
		seria_kv("port", v.port, s);
	}

	void ser_members(cryonero::connection_entry &v, seria::ISeria &s) {
		seria_kv("adr", v.adr, s);
		seria_kv("id", v.id, s);
		seria_kv("is_income", v.is_income, s);
	}

	void ser_members(cryonero::CoreStatistics &v, seria::ISeria &s) {
	...
	void ser_members(cryonero::COMMAND_REQUEST_STAT_INFO::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }
	...
	void ser_members(cryonero::COMMAND_REQUEST_NETWORK_STATE::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }
	...
	void ser_members(cryonero::RawBlockLegacy &v, seria::ISeria &s) {
		...
		if (s.is_input()) {
			...
			std::transform(other_transactions.begin(), other_transactions.end(), std::back_inserter(v.transactions),
				[](const std::string &s) { return cryonero::BinaryArray(s.data(), s.data() + s.size()); });
		}
		else {
			...
			std::transform(v.transactions.begin(), v.transactions.end(), std::back_inserter(other_transactions),
				[](cryonero::BinaryArray &s) { return std::string(s.begin(), s.end()); });
			seria::seria_kv("block", other_block, s);
			seria::seria_kv("txs", other_transactions, s);
		}
	}
	...
	void ser_members(cryonero::NOTIFY_NEW_TRANSACTIONS::request &v, seria::ISeria &s) {
		...
		if (s.is_input()) {
			...
			std::transform(v.txs.begin(), v.txs.end(), std::back_inserter(transactions),
				[](const cryonero::BinaryArray &s) { return std::string(s.begin(), s.end()); });
			seria::seria_kv("txs", transactions, s);
		}
	}
