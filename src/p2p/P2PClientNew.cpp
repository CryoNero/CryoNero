...
cryonero::P2PClientNew::HandlerFunction handler_method(void (cryonero::P2PClientNew::*handler)(Cmd &&)) {
...
void P2PClientNew::on_request_ready() {
	...
	while (read_next_request(binary_header, body)) {
		try {
			...
			std::cout << "Skipping unknown cryonero::P2P cmd=" << header.command << " size=" << header.body_size
				<< std::endl;
		}
