...
static void parse_peer_and_add_to_container(const std::string &str, std::vector<cryonero::NetworkAddress> &container) {
	cryonero::NetworkAddress na{};
...
const static UUID cryonero_NETWORK = {{0xb7, 0x09, 0xf9, 0x85, 0x85, 0x39, 0xf1, 0x87, 0xde, 0x5d, 0x6c, 0x02, 0x48, 0x7c, 0xaa, 0x57}};  // Bender's nightmare
Config::Config(common::CommandLine &cmd)
	...
	if (seed_nodes.empty() && !is_testnet)
		for (auto &&sn : cryonero::SEED_NODES) {
			NetworkAddress addr;
