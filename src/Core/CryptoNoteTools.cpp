...
Hash cryonerocoin::get_base_transaction_hash(const BaseTransaction &tx) {
	if (tx.version < 2)
...
void cryonerocoin::fix_merge_mining_tag(BlockTemplate &block) {
	if (block.major_version >= 2) {
		cryonerocoin::TransactionExtraMergeMiningTag mmTag;
		mmTag.depth = 0;
		block.parent_block.base_transaction.extra.clear();
		mmTag.merkle_root = get_auxiliary_block_header_hash(block);
		if (!cryonerocoin::append_merge_mining_tag_to_extra(block.parent_block.base_transaction.extra, mmTag))
			throw std::runtime_error("cryonerocoin::append_merge_mining_tag_to_extra failed");
	}
}
...
void cryonerocoin::decompose_amount(Amount amount, Amount dust_threshold, std::vector<Amount> *decomposed_amounts) {
...
size_t cryonerocoin::get_maximum_tx_size(size_t input_count, size_t output_count, size_t mixin_count) {
...
bool cryonerocoin::get_tx_fee(const TransactionPrefix &tx, uint64_t *fee) {
...
uint64_t cryonerocoin::get_tx_fee(const TransactionPrefix &tx) {
	uint64_t r = 0;
	if (!get_tx_fee(tx, &r))
		return 0;
	return r;
