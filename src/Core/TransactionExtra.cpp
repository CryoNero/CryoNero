...
static void do_serialize(cryonerocoin::TransactionExtraMergeMiningTag &tag, seria::ISeria &s) {
	s.begin_object();
...
void seria::ser(cryonerocoin::TransactionExtraMergeMiningTag &v, ISeria &s) {
	if (s.is_input()) {
		std::string field;
		s(field);
